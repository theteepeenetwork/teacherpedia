    ...
    public static function catalog(): array
    {
        $base = [
            ...
            ['slug' => 'digit-detectives', 'name' => 'Digit Detectives',
             'description' => 'A correct column addition with some digits hidden in lettered boxes. Solvers run the written method backwards — using place value and carries — to recover each missing digit, then read them through a cipher to spell a secret word. The word only spells correctly if every digit is right, so it self-checks. Three challenge levels.',
             'blurb' => 'Digit Detectives turns formal column addition into a deduction puzzle. A real, correct sum is printed in the written layout, but several interior digits are hidden behind lettered boxes (A, B, C…). There is never any guessing: the digits you can see, the total, and the carries between columns force every hidden digit to exactly one value. Look each recovered digit up in the Detective\'s Codebook to turn it into a letter, then read the letters in order to reveal a secret word at the foot of the sheet — a word that only comes out right when every digit is correct, so the puzzle quietly marks itself.',
             'icon' => '🔍', 'tags' => 'Puzzle,Self-marking,Reasoning,Place value', 'status' => 'live', 'route' => '/digit-detectives', 'sort_order' => 10,
             'min_year' => 3, 'max_year' => 6,
             'image' => '/assets/images/resources/digit-detectives.png',
             'how' => [
                 'Each puzzle is a real column addition with some digits hidden in lettered boxes.',
                 'Work right to left: the digits you can see, the total, and the carries force each hidden digit to one value — no guessing.',
                 'Look up each digit you find in the Detective\'s Codebook to turn it into a letter.',
                 'Read the letters in order A, B, C… along the footer to reveal the secret word.',
                 'The word only spells correctly if every digit is right — so it marks itself.',
             ]],
