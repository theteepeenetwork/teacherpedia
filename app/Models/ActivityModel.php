    ...
    public static function catalog(): array
    {
        $base = [
            ...
            ['slug' => 'digit-detectives', 'name' => 'Digit Detectives',
             'description' => 'A grid of small column additions, each a correct sum with exactly two digits hidden in two different columns. Solvers run the written method backwards — using place value and carries — to recover the missing digits, then read the two as a code (00=A … 25=Z) to decode one letter per puzzle. The letters spell a joke punchline or praise word, so the sheet self-marks. Magnitude scales by year; difficulty fine-tunes within it.',
             'blurb' => 'Digit Detectives turns formal column addition into a whole-class code hunt. Each card is a real, correct sum printed in the written layout, but two interior digits — in two different columns — are hidden behind outlined boxes. There is never any guessing: because only one digit is hidden per column and everything else in that column is shown, the visible digits, the total and the carries force each blank to exactly one value. Read the two recovered digits left-to-right as a two-digit code and look it up in the Detective\'s Codebook (00=A, 01=B, … 25=Z) to decode one letter. One letter per puzzle, in puzzle order, spells a joke punchline or a praise word across the whole sheet — a message that only comes out right when every digit is correct, so the puzzle quietly marks itself.',
             ...
             'how' => [
                 'Each puzzle is a real column addition with exactly two digits hidden — one in each of two different columns.',
                 'Work right to left: because every other digit in a hidden digit\'s column is shown, the visible digits, the total and the carries force each blank to one value — no guessing.',
                 'Read the two recovered digits left-to-right as a two-digit code, then look it up in the Detective\'s Codebook (00=A, 01=B, … 25=Z) to decode one letter.',
                 'Write one letter per puzzle, in order, along the footer to reveal the joke punchline or praise word.',
