    ...
    public static function catalog(): array
    {
        $base = [
            ...
            ['slug' => 'spot-the-impostor', 'name' => 'Spot the Impostor',
             'description' => 'A grid of calculations already worked out — but some answers are wrong, each made with a real pupil mistake (a forgotten carry, smaller-from-larger, an off-by-one table fact). Children judge every cell ✓ or ✗, correct the impostors, then add the correct answers and self-check against an honest-total footer. The Answer key names the misconception behind each impostor. Years 1–6, the one resource that asks pupils to evaluate, not compute.',
             'blurb' => 'Spot the Impostor flips the worksheet: instead of working calculations out, children judge whether worked answers are right. Each cell shows a calculation with its answer already filled in — but some are deliberately wrong, and crucially wrong in the way a real child would get them wrong: a forgotten carry (47 + 38 = 75), taking the smaller digit from the larger (52 − 27 = 35), adding instead of multiplying (6 × 4 = 10), slipping a row in a times table, dropping a remainder, rounding a half down, treating a decimal as a whole number. Nothing on the worksheet marks which are impostors. Pupils tick the ones they trust, cross the impostors and write the correction, then add up all the correct answers — if they land on the honest total printed in the footer, they have probably caught every impostor (a strong check, not proof). The Answer key tab reveals each cell as Correct or Impostor and names the exact misconception behind every wrong answer, the perfect discussion prompt. Built for Years 1 to 6: the year and operations gate the number ranges and the bugs on offer, so a Year 1 sheet judges add/subtract within 20 while a Year 6 sheet can include long division, rounding in context and decimals. Every wrong answer is reasoning made visible — the resource that teaches children to evaluate.',
             ...
             'how' => [
                 'Choose the year group, the operations, and how many impostors hide in the grid.',
                 'Every cell shows a calculation already worked out — but some answers are wrong, made with a real pupil mistake (a forgotten carry, smaller-from-larger, an off-by-one table fact…).',
                 'For each cell, decide: is it right (✓) or an impostor (✗)? Mark the ✗ ones and write the correct answer.',
                 'Add up all the correct answers. If you land on the honest total in the footer, you have probably caught every impostor — a match is a strong check, not proof.',
                 'Use the Answer key tab to reveal each impostor and name the mistake — the perfect discussion prompt.',
             ]],
