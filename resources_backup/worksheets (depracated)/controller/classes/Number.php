...
public
    ...
    function place_value()
        ...
        $length = strlen($a);
        $qtn = "In " . number_format($a) . " what is the digit in the " . $place[$selector] . " column?" . $selector;
        $answr = $array[$length - $selector - 1];
        return $this->make_array($qtn, $answr);
        }
    ...
    public function orderFractionsSameDenoms() {
        ...
        for($i = 0; $i < 6; $i++) {
            ...
            array_push($fractionsToSort[$fractionString]=$decimal);
        }
